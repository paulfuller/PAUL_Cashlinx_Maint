...
{
    public partial class ManageOverrides : Form
    {
        ...
        public ManageOverrides(string trigger)
        {
        ...
        public ManageOverrides(string trigger, decimal overrideAmount)
        {
            this.trigger = trigger;
            InitializeComponent();
            _OverrideAmount = new List<decimal>();
        ...
        public ManageOverrides(string trigger, int iTransactionNumber, decimal overrideAmt, decimal suggLoanAmt, ManagerOverrideTransactionType managerOverrideTransactionType, ManagerOverrideType managerOverrideType)
        {
            this.trigger = trigger;
            InitializeComponent();
            _OverrideAmount = new List<decimal>();
        ...
        private void ManageOverrides_Load(object sender, EventArgs e)
        {
                ...
                case DESCRIBEMERCH_TRIGGER:
                    ...
                    foreach (ManagerOverrideTransactionType type in ManagerOverrideTransactionTypes)
                    {
                        ...
                        else
                        {
                            if (difference < 0)
                            {
                                _infoMessage = Commons.GetMessageString("ManageOverrideLoanMsg") + _OverrideAmount[0] + " " + Commons.GetMessageString("ManageOverrideDescMercMsgLess") + System.Environment.NewLine + Commons.GetMessageString("ManageOverrideDescMercMsg2") + diff.ToString() + "!" + System.Environment.NewLine + Commons.GetMessageString("ManageOverrideMgrMsg");
                            }
                            else
                            {
                                _infoMessage = Commons.GetMessageString("ManageOverrideLoanMsg") + _OverrideAmount[0] + " " + Commons.GetMessageString("ManageOverrideDescMercMsgMore") + System.Environment.NewLine + Commons.GetMessageString("ManageOverrideDescMercMsg2") + difference.ToString() + "!" + System.Environment.NewLine + Commons.GetMessageString("ManageOverrideMgrMsg");
                            }
                        }
            ...
            if (showReasonCombobox)
            {
                ...
                reasonComboBox.DataSource = GlobalDataAccessor.Instance.DesktopSession.ManagerOverrideReasonCodes;
        ...
        private void userNameTextbox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                GlobalDataAccessor.Instance.DesktopSession.ScanParse(this, txt, userNameTextbox, passwordTextBox, submitButton);
            }
        ...
        private void submitButton_Click(object sender, EventArgs e)
        {
            ...
            var overridingUserId = userNameTextbox.Text.Trim();
            ...
            try
            {
                if (callLdap(userName, userPassword))
                {
                    ...
                    if (checkRole(userName))
                    {
                        ...
                        auditLogMap.Add(DesktopSession.AUDIT_STORENUMBER, GlobalDataAccessor.Instance.CurrentSiteId.StoreNumber);
                        auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_ID, overridingUserId);
                        if (ManagerOverrideTransactionTypes.Count > 0)
                        {
                            int auditArraySize = ManagerOverrideTransactionTypes.Count;

                            auditLogMap.Add(DesktopSession.AUDIT_CARDINALITY, auditArraySize);
                            auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_TRANS_TYPE, ManagerOverrideTransactionTypes.ToArray());
                            auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_TYPE, ManagerOverrideTypes.ToArray());
                            auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_SUGGVAL, _SuggestedLoanAmount.ToArray());
                            auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_APPRVAL, _OverrideAmount.ToArray());
                            auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_TRANSNUM, TransactionNumbers.ToArray());
                            ...
                            if (showReasonCombobox)
                            {
                                ...
                                auditLogMap.Add(DesktopSession.AUDIT_OVERRIDE_COMMENT, managerOverrideReason.ToString());
                            }
                        }
        ...
        private void submitButton_EnabledChanged(object sender, EventArgs e)
        {
            ...
            {
                if (CashlinxDesktopSession.Instance.ScannedCredentials)
                {
                    CashlinxDesktopSession.Instance.ScannedCredentials = false;
