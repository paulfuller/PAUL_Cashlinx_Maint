...
namespace Common.Libraries.Utility.BarcodeGenerator.Symbologies
{
    class UPCE : BarcodeCommon, IBarcode
    {
        ...
        {
            if (Raw_Data.Length != 6 && Raw_Data.Length != 8 && Raw_Data.Length != 12) throw new System.Exception("EUPCE-1: Invalid data length. (8 or 12 numbers only)");

            if (!Barcode.CheckNumericOnly(Raw_Data)) throw new System.Exception("EUPCE-2: Numeric only.");
