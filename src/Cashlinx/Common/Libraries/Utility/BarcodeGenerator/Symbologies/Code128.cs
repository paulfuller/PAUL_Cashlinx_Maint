    ...
    class Code128 : BarcodeCommon, IBarcode
    {
        ...
        private List<DataRow> FindStartorCodeCharacter(string s, ref int col)
        {
            ...
            else
            {
                ...
                foreach (DataRow row in this.C128_Code.Rows)
                {
                    ...
                    {
                        throw new System.Exception("EC128-1: " + ex.Message);
                    }//catch
                }//foreach                

                if (rows.Count <= 0)
                    throw new System.Exception("EC128-2: Could not determine start character.");
            }//else
        ...
        private void BreakUpDataForEncoding()
        {
            ...
            if (this.type == TYPES.C)
            {
                ...
                {
                    throw new System.Exception("EC128-6: Only numeric values can be encoded with C128-C.");
                }//catch
            ...
            if (this.type != TYPES.DYNAMIC)
            {
                switch (this.type)
                {
                    ...
                    case TYPES.C: _FormattedData.Insert(0, "START_C");
                        break;
                    default: throw new System.Exception("EC128-4: Unknown start type in fixed type encoding.");
                }
            }//if
            else
            {
                                ...
                                while (error)
                                {
                                    ...
                                    { 
                                        ...
                                        if (col++ > CurrentCodeSet.ItemArray.Length)
                                            throw new System.Exception("No start character found in CurrentCodeSet.");
                                    }//catch
                                }//while
                ...
                {
                    throw new System.Exception("EC128-3: Could not insert start and code characters.\n Message: " + ex.Message);
                }//catch
            }//else
        }
        private string GetEncoding()
        {
            ...
            foreach (string s in _FormattedData)
            {
                ...
                if (E_Row == null || E_Row.Length <= 0)
                    throw new System.Exception("EC128-5: Could not find encoding of a value( " + s1 + " ) in C128 type " + this.type.ToString());
