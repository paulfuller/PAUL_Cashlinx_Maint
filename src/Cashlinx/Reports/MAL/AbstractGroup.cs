...
namespace Reports.MAL
{
    public abstract class AbstractGroup
    {
        ...
        protected ReportContext Context { get; set; }
        ...
        protected string ErrorCode
        {
            ...
            set { Context.ErrorCode = value; }
        }

        protected string ErrorText
        {
            get { return Context.ErrorText; }
            set { Context.ErrorText = value; }
        }

        public AbstractGroup(ReportContext theContext, string dataTableName)
        {
            Context = theContext;
            HeaderCells = new List<PdfPCell>();
            Document = Context.Document;
            PdfTable = Context.PdfTable;
        ...
        {
            DataTable dataTable;
            DataLoaded = Context.ReportData.GetTable(dataTableName, out dataTable);

            GroupData = dataTable;
        }
        //protected virtual void PrintGroupDetailRecord(PdfPTable pdfTable, int index, DataRow dataRow, bool shadeRow)
        protected virtual void PrintGroupDetailRecord(PdfPTable pdfTable, DataView data)
        {
        }
        ...
        protected void PrintGroupDetail(PdfPTable pdfTable)
        {
            ...
            var groupTransDT = from p in GroupData.AsEnumerable()
                               orderby Convert.ToDateTime(p.Field<string>("trans_dt")),
                               p.Field<string>("CUST_NR") ascending
                               select p;
            DataView t = groupTransDT.AsDataView();
           
            //here sort table
            string lastTransDate = string.Empty;
            //for (int i = 0; i < t.Table.Rows.Count; i++)
            //{
                //string currentTransDate = t.Table.Rows[i]["trans_dt"].ToString();           
                //if (currentTransDate != lastTransDate)
                //{
                    //add new header
                    //pdfTable.AddCell(GetCell(currentTransDate, DataFontBold, Element.ALIGN_LEFT, false, 12, 0));
                //}
                //PrintGroupDetailRecord(pdfTable, i, t.Table.Rows[i], !(i % 2 == 0));
            PrintGroupDetailRecord(pdfTable, t);
                //lastTransDate = currentTransDate;
