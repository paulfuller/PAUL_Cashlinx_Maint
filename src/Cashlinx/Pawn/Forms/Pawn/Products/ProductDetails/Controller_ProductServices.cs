...
namespace Pawn.Forms.Pawn.Products.ProductDetails
{
    public partial class Controller_ProductServices : Form
    {
        ...
        private bool ContinueAfterBackgroundChecked()
        {
            ...
            /* DateTime currentDate = ShopDateTime.Instance.ShopDate;
             ...
             if (currentCustomer.HasValidConcealedWeaponsPermitInState(strStoreState, currentDate))
             {
                 ...
                 {
                     CashlinxDesktopSession.Instance.BackgroundCheckCompleted = true; //If the background check is not needed
                 }
             }
                ...
                if (GlobalDataAccessor.Instance.DesktopSession.BackgroundCheckCompleted || _updateBuyoutStatus)
                {
                    ...
                    if (selectedLoans.Count > 0)
                    {
                        ...
                        if (GlobalDataAccessor.Instance.DesktopSession.TotalBuyoutAmount != 0 || GlobalDataAccessor.Instance.DesktopSession.TotalOtherPickupItems != 0)
                        {
                            ...
                            //CashlinxDesktopSession.Instance.TotalPickupAmount = _totalPickupAmount;
                        }
                    }
                    else
                    {
        ...
        private void DisableAllServiceButtons()
        {
            ...
            //PS_WaiveProrate.Enabled = CashlinxDesktopSession.Instance.ServiceLoans.Count > 0;
        ...
        private void LayawayCheckout()
        {
            ...
            List<LayawayVO> servicedLayaways = CashlinxDesktopSession.Instance.ServiceLayaways;
            ...
            if (pickupLayaways.Count > 0)
            {
                ...
                if (!CashlinxDesktopSession.Instance.CompleteLayaway)
                {
                    foreach (LayawayVO layaway in pickupLayaways)
                    {
                        ...
                        if (gunItems.Count > 0)
                        {
                            ...
                            if (!LayawayProcedures.CustomerPassesFirearmAgeCheckForItems(layaway, CashlinxDesktopSession.Instance.ActiveCustomer))
                            {
                                CashlinxDesktopSession.Instance.CompleteLayaway = false;
                    ...
                    if (_gunItem && !ContinuePickup)
                    {
                        ...
                        CashlinxDesktopSession.Instance.OverrideTransactionNumbers = overrideTransactionNumbers;
                        this.NavControlBox.IsCustom = true;
                ...
                if (_gunItem && CashlinxDesktopSession.Instance.BackgroundCheckFeeValue > 0)
                {
                    decimal[] feeValues = Utilities.GetDistributeValuesForCurrencyOverItems(CashlinxDesktopSession.Instance.BackgroundCheckFeeValue, pickupLayawaysWithGun.Count);
            ...
            decimal totalServiceAmount = servicedLayaways.SelectMany(lay => lay.Payments).Sum(layPmt => layPmt.Amount) + CashlinxDesktopSession.Instance.BackgroundCheckFeeValue;
            CashlinxDesktopSession.Instance.TenderTransactionAmount.TotalAmount = totalServiceAmount;
            CashlinxDesktopSession.Instance.TenderTransactionAmount.SubTotalAmount = totalServiceAmount;
            CashlinxDesktopSession.Instance.TenderTransactionAmount.SalesTaxPercentage = 0.0M;
            CashlinxDesktopSession.Instance.DisableCoupon = true;
        ...
        private void LoanCheckout()
        {
            ...
            if (CashlinxDesktopSession.Instance.TotalServiceAmount > 0)
            {
                TenderCash cashTenderForm = new TenderCash();
                cashTenderForm.ShowDialog();
                if (CashlinxDesktopSession.Instance.CashTenderFromCustomer > 0)
                    cashProcessed = true;
            }
            else
            {
            ...
            if (cashProcessed)
            {
                ...
                CashlinxDesktopSession.Instance.ActivePawnLoan = null;
        ...
        private void LoadData(ProductType productType)
        {
            ...
            {
                _LoanKeys = (from loankey in GlobalDataAccessor.Instance.DesktopSession.PawnLoanKeys
                             where loankey.LoanStatus == ProductStatus.IP
                             select loankey).ToList();
                _AuxillaryLoanKeys = (from loankey in GlobalDataAccessor.Instance.DesktopSession.PawnLoanKeysAuxillary
                                      where loankey.LoanStatus == ProductStatus.IP
                                      select loankey).ToList();
            }
            ...
            extensionServiceAllowed = new BusinessRulesProcedures(GlobalDataAccessor.Instance.DesktopSession).IsExtensionAllowed(currentStoreSiteId);
        ...
        private void PS_RollOverButton_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < cnt; ++i)
            {
                ...
                /* if (p == null ||
                rolloverForm.SkippedPawnLoans.Contains(p))
                {
                //Remove skipped loan from rollover loans list
                rolloverForm.RolloverPawnLoans.RemoveAt(idx);

                //NOTE: We do not increment idx here as the RemoveAt
                //operation will shift the remaining loans backward and
                //the next loan to operate on will be at the same idx value
                continue;
                }*/
                ShowStatusValue(p);
