...
{
    public partial class Print_PfiChargeOffList : Form
    {
        ...
        public Print_PfiChargeOffList(DataTable dataTable, string sTotalCost, string sTags, string sAsOf)
        {
            InitializeComponent();
        ...
        {
            // Number
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colNumber"], "")), 0, iRowIdx);
            // Ticket Description
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colDescription"], "")), 1, iRowIdx);
            // Tags
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colTags"], "")), 2, iRowIdx);
            // Cost
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colCost"], "")), 3, iRowIdx);
            // Retail
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colRetail"], "")), 4, iRowIdx);
            // Reason
            tlpRecords.Controls.Add(CreateColumn(Utilities.GetStringValue(dataRow["colReason"], "")), 5, iRowIdx);

        }

        private Label CreateColumn( string sText)
        {
            Font font = new Font(LoanNoColumnLabel.Font, FontStyle.Regular);
            Label label = Utilities.CloneObject<Label>(LoanNoColumnLabel);
            label.AutoSize = true;
            label.Font = font;
            label.Text = sText;
            return label;
