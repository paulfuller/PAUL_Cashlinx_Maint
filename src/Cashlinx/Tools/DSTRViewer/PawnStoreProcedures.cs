    ...
    public static class PawnStoreProcedures
    {
            ...
            out string guid)
        {
            ...
            bool retVal = DataAccessService.ExecuteStoredProc(
                ...
                "o_return_code", "o_return_text", CCSOWNER,
            ...
            out ulong nextId)
        {
            ...
            if (!DataAccessService.ExecuteVariableQuery(false,
                    ...
                    PAWNSEC, out dataReturnSet, ref dA, parmArr))
            {
            ...
            out ulong nextId)
        {
            ...
            if (!DataAccessService.ExecuteVariableQuery(false,
                    ...
                    CCSOWNER, out dataReturnSet, ref dA, parmArr))
            {
                dA.log(LogLevel.ERROR, "Could not acquire the next valid id from the {0} column of the {1} table",
                    idColumnName, tableName);
                return (false);
            ...
            string StoreNum)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve storesiteinfo for the Store -->" + StoreNum, "Alert");
                return false;
            }
        ...
        string StoreNum, string nextNumType)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve CCSOWNER.NEXTNUM for the Store -->" + StoreNum, "Alert");
                return false;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
        ...
        ref string safeId, ref string registeredUserId)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve Safe for -->" + safeName, "Alert");
                return;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
                MessageBox.Show("Safe not available for the Store-->" + safeName, "Alert");
                return;
            }
        ...
        string registeredUserId)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve cashdraweruser for the user-->" + registeredUserId, "Alert");
                return false;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
            ...
            string StoreNum)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve storetypes for the Store -->" + StoreNum, "Alert");
                return false;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
        ...
        ref List<string> nextNums)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve NextNum for the Store -->" + StoreNum, "Alert");
                return;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
                MessageBox.Show("No NextNum records available for the Store-->" + StoreNum, "Alert");
                return;
            }
            ...
            ref List<PairType<string, string>> storeProdcuts)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not retrieve store products for the Store -->" + StoreNum, "Alert");
                return;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
                MessageBox.Show("No Store Products available for the Store-->" + StoreNum, "Alert");
                return;
            }
            ...
            ref List<PeripheralModelVO> peripheralModels)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not acquire peripheral models", "Alert");
                return;
            }
            if (returnSet == null || returnSet.NumberRows <= 0)
            {
                MessageBox.Show("No peripheral models returned", "Alert");
                return;
            }
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                           ...
                                           ref dA))
            {
                MessageBox.Show("Could not acquire peripheral types", "Alert");
                return;
            }
            if (perReturnSet == null || perReturnSet.NumberRows <= 0)
            {
                MessageBox.Show("No peripheral types returned", "Alert");
                return;
            }
            ...
            ref List<PeripheralVO> peripherals)
        {
            ...
            if (!DataAccessService.ExecuteQuery(false,
                                                ...
                                                ref dA))
            {
                MessageBox.Show("Error occurred while acquiring peripherals.", "Alert");
                return;
            }
            ...
            {
                MessageBox.Show("No peripherals returned.", "Alert");
                return;
            }
            ...
            ref Dictionary<string, List<PairType<string, PeripheralVO>>> wkspMapping)
        {
            ...
            {
                MessageBox.Show("No workstation peripheral mappings returned.", "Alert");
                return;
            }
            ...
            ref List<QuadType<UserVO, LDAPUserVO, string, string>> users)
        {
            ...
            {
                MessageBox.Show("No users found for facility " + facFilter, "Alert");
                return;
            }
            ...
            out bool changedPublicKey)
        {
            ...
            if (!string.Equals(globPubKeyDec, pwnSec.GlobalConfiguration.DataPublicKey))
            {
                var res =
                        MessageBox.Show(
                                string.Format("The public key stored in the database does not match {0}the public key generated in this setup session.  Are you sure you want to change the {0}public key?", Environment.NewLine),
                                "Alert",
