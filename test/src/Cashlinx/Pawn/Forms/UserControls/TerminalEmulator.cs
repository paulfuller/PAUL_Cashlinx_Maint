...
namespace Pawn.Forms.UserControls
{
    public class TerminalEmulator : Control
    {
        ...
        public bool DataEventNotify { get; set; }
        ...
        public bool Connect()
        {
            switch (this.ConnectionType)
            {
                case ConnectionTypes.Telnet:
                    {
                        return this.ConnectTelnet(this.Hostname);
                    }
                ...
                case ConnectionTypes.SSH2:
                    {
                        return this.ConnectSSH2(this.Hostname, this.Username, this.Password);
                    }
                default:
                    {
                        return false;
        ...
        #endregion
        #region Delegates
        public delegate void RawDataEventHandler(string sRawData);
        ...
        #endregion
        #region Events
        public event RawDataEventHandler RawDataEvent;
        public event RawDataEventHandler CleanDataEvent;
        ...
        public TerminalEmulator()
        {
            ...
            VertScrollBar.Maximum = 100;
            VertScrollBar.Value = 100;
        ...
        private bool ConnectTelnet(string HostName)
        {
        ...
        private bool ConnectSSH2(string hostname, string username, string password)
        {
            bool bConnected = false;
            // connect ssh
            this.Focus();
            CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnection _conn;
            CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnectionParameter f = new CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnectionParameter();
            f.UserName = username;
            f.Password = password;
            f.Protocol = CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHProtocol.SSH2;

            f.AuthenticationType = CashlinxDesktop.UserControls.TerminalControl.SSHC.AuthenticationType.Password;
            f.WindowSize = 0x1000;
            ...
            _conn = CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnection.Connect(f, reader, s);
            reader._conn = _conn;
            CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHChannel ch = _conn.OpenShell(reader);
            reader._pf = ch;
            CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnectionInfo ci = _conn.ConnectionInfo;
            return bConnected;
        }
        ...
        private void HandleScroll(Object sender, ScrollEventArgs se)
        {
            ...
            switch (se.Type)
            {
                ...
                case ScrollEventType.ThumbTrack:
                    int iOldValue = se.OldValue;
                    int iNewValue = se.NewValue;
                    if (iNewValue != iOldValue)
                    {
                        this.LastVisibleLine += iNewValue - iOldValue;
                    }
                    break;
                default:
                    return;
        ...
        private void OnReceivedData(System.IAsyncResult ar)
        {
            ...
            if (nBytesRec > 0)
            {
                var sReceived = string.Empty;
                if (this.DataEventNotify) RawDataEvent("CLEAR");
                if (this.DataEventNotify) CleanDataEvent("CLEAR");
                ...
                if (DataEventNotify) RawDataEvent(sReceived);

                this.Invoke(this.RxdTextEvent, new System.String[] { System.String.Copy(sReceived) });
                this.Invoke(this.RefreshEvent);

                // Re-Establish the next asyncronous receveived data callback as
                StateObject.Socket.BeginReceive(StateObject.Buffer, 0, StateObject.Buffer.Length, SocketFlags.None, new AsyncCallback(OnReceivedData), StateObject);
            }
            else
            {
        ...
        private void PrintChar(System.Char CurChar)
        {
            ...
            if (this.DataEventNotify) CleanDataEvent(CurChar.ToString());
        ...
        private void ShowChar(System.Char CurChar, System.Int32 Y, System.Int32 X, CharAttribStruct CurAttribs)
        {
                ...
                (CurBGColor != this.FGColor && (this.Modes.Flags & uc_Mode.LightBackground) > 0))
            {
                ...
                _CurGraphics.DrawImageUnscaled(
                    this.EraseBitmap,
                    X,
                    Y);
            }
            ...
            {
                _CurGraphics.DrawLine(new System.Drawing.Pen(CurFGColor, 1),
            ...
            if (X != this.DrawStringOffset.X && Y != this.DrawStringOffset.Y)
                _CurGraphics.DrawString(
                    CurChar.ToString(),
                    this.Font,
        ...
        private void WipeScreen()
        {
            ...
            if ((this.Modes.Flags & uc_Mode.LightBackground) > 0)
            {
                _CurGraphics.Clear(this.FGColor);
            }
            else
            {
                _CurGraphics.Clear(this.BackColor);
            }
        }
        ...
        private void CommandRouter(object Sender, ParserEventArgs e)
        {
            ...
            switch (e.CurSequence)
            {
                ...
                case "\x1b;":
                    ...
                    if (e.CurParams.Count() > 0)
                    {
                        yCur = Convert.ToInt32(e.CurParams.Elements[0]);
                        xCur = Convert.ToInt32(e.CurParams.Elements[1]);
                    }

                    this.Caret.Pos.X = xCur;
                    this.Caret.Pos.Y = yCur;
            ...
            public static System.Char Get(System.Char CurChar, uc_Chars.Sets GL, uc_Chars.Sets GR)
            {
                ...
                for (System.Int32 i = 0; i < CurSet.Length; i++)
                {
                    if (CurSet[i].Location == System.Convert.ToInt32(CurChar))
                    {
                        ...
                        //                        System.Char[] NewChars = System.Text.Encoding.Default.GetChars(CurBytes);
        ...
        private class uc_Keyboard
        {
            ...
            private class uc_KeyMap
            {
                ...
                public void SetToDefault()
                {
                    ...
                    // Corrections by Rusty Davis
                    //                    Elements.Add(new uc_KeyInfo(59, false, "Key", "\x1B[X", uc_Mode.Any, 0)); //F1->PF1
                    //                    Elements.Add(new uc_KeyInfo(60, false, "Key", "\x1B[Y", uc_Mode.Any, 0)); //F2->PF2
                    //                    Elements.Add(new uc_KeyInfo(61, false, "Key", "\x1B[Z", uc_Mode.Any, 0)); //F3->PF3
                    //                    Elements.Add(new uc_KeyInfo(62, false, "Key", "\x1B[a", uc_Mode.Any, 0)); //F4->PF4
                    //                    Elements.Add(new uc_KeyInfo(63, false, "Key", "\x1B[b", uc_Mode.Any, 0)); //F5
                    Elements.Add(new uc_KeyInfo(64, false, "Key", "\x1B[c", uc_Mode.Any, 0)); //F6
                    Elements.Add(new uc_KeyInfo(65, false, "Key", "\x1B[d", uc_Mode.Any, 0)); //F7
                    Elements.Add(new uc_KeyInfo(66, false, "Key", "\x1B[e", uc_Mode.Any, 0)); //F8
                    Elements.Add(new uc_KeyInfo(67, false, "Key", "\x1B[f", uc_Mode.Any, 0)); //F9
                    Elements.Add(new uc_KeyInfo(68, false, "Key", "\x1B[h", uc_Mode.Any, 0)); //F10

                    // TOPS over-ride
                    Elements.Add(new uc_KeyInfo(59, false, "Key", "\x1bu", uc_Mode.Any, 0)); //F1->PF1
                    Elements.Add(new uc_KeyInfo(60, false, "Key", "\x1BnF", uc_Mode.Any, 0)); //F2->PF2
                    Elements.Add(new uc_KeyInfo(61, false, "Key", "\x1Bh", uc_Mode.Any, 0)); //F3->PF3
                    Elements.Add(new uc_KeyInfo(62, false, "Key", "\x1Bi", uc_Mode.Any, 0)); //F4->PF4
                    Elements.Add(new uc_KeyInfo(63, false, "Key", "\x1BpF", uc_Mode.Any, 0)); //F5
        ...
        private class uc_Parser
        {
            ...
            private class uc_CharEvents
            {
                ...
                public static uc_CharEventInfo[] Elements = 
                {
                    ...
                    // new			new uc_CharEventInfo (States.Anywhere,      '\x9C', '\x9C', Actions.None,    States.Ground),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.EscapeIntrmdt, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    //             new uc_CharEventInfo (States.CsiEntry,      '\x3b', '\x3b', Actions.Collect,   States. CsiEntry), //Looking for “;”
                    //             new uc_CharEventInfo (States.CsiEntry,      '\x48', '\x48', Actions.Collect,   States. Ground),   //Looking for “H”
                    new uc_CharEventInfo(States.CsiEntry, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.Escape, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // new				new uc_CharEventInfo (States.Escape,        '\x50', '\x50', Actions.None,    States.DcsEntry),
                    ...
                    // new				new uc_CharEventInfo (States.Escape,        '\x5B', '\x5B', Actions.None,    States.CsiEntry),
                    ...
                    new uc_CharEventInfo(States.Escape, '\x59', '\x59', Actions.Dispatch, States.Ground),
                    new uc_CharEventInfo(States.Escape, '\x5A', '\x5A', Actions.Dispatch, States.Ground),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.CsiParam, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.CsiIgnore, '\x20', '\x3F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.CsiIgnore, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.CsiIntrmdt, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.SosPmApcString, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.SosPmApcString, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.SosPmApcString, '\x1C', '\x1F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.SosPmApcString, '\x20', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.DcsEntry, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsEntry, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsEntry, '\x1C', '\x1F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsEntry, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.DcsIntrmdt, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIntrmdt, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIntrmdt, '\x1C', '\x1F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIntrmdt, '\x20', '\x2F', Actions.Collect, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIntrmdt, '\x7F', '\x7F', Actions.Ignore, States.None),
                    new uc_CharEventInfo(States.DcsIgnore, '\x9C', '\x9C', Actions.None, States.Ground),
                    // Added below
                    new uc_CharEventInfo(States.DcsIgnore, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIgnore, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIgnore, '\x1C', '\x1F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsIgnore, '\x20', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.DcsParam, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsParam, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsParam, '\x1C', '\x1F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsParam, '\x7F', '\x7F', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.DcsParam, '\x40', '\x7E', Actions.None, States.DcsPassthrough),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.DcsPassthrough, '\x7F', '\x7F', Actions.Ignore, States.None),
                    ...
                    // Added below
                    new uc_CharEventInfo(States.OscString, '\x00', '\x17', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.OscString, '\x19', '\x19', Actions.Ignore, States.None),
                    // Added below
                    new uc_CharEventInfo(States.OscString, '\x1C', '\x1F', Actions.Ignore, States.None)
                };
            }
        }
    ...
    class Reader : CashlinxDesktop.UserControls.TerminalControl.SSHC.ISSHConnectionEventReceiver, CashlinxDesktop.UserControls.TerminalControl.SSHC.ISSHChannelEventReceiver
    {
        ...
        public CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHConnection _conn;
        public bool _ready;
        ...
        public CashlinxDesktop.UserControls.TerminalControl.SSHC.PortForwardingCheckResult CheckPortForwardingRequest(string host, int port, string originator_host, int originator_port)
        {
            CashlinxDesktop.UserControls.TerminalControl.SSHC.PortForwardingCheckResult r = new CashlinxDesktop.UserControls.TerminalControl.SSHC.PortForwardingCheckResult();
            r.allowed = true;
            r.channel = this;
            return r;
        }

        public void EstablishPortforwarding(CashlinxDesktop.UserControls.TerminalControl.SSHC.ISSHChannelEventReceiver rec, CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHChannel channel)
        {
            _pf = channel;
        }

        public CashlinxDesktop.UserControls.TerminalControl.SSHC.SSHChannel _pf;
    }
    #endregion
}
