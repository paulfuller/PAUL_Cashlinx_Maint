...
{
    public class StatusChangeReport : ReportBase
    {
        #region Fields
        ...
        private void WriteDetail(PdfPTable detailsTable, Int32 columnCount, DataSet statusChangeDataSet)
        {
            DataTable statusChangeTable = statusChangeDataSet.Tables[0];

            for (int i = 0; i < statusChangeTable.Rows.Count; i++)
            {
                System.Data.DataRow statusChange = statusChangeTable.Rows[i];

                WriteCell(detailsTable, (i + 1).ToString() + @")", ReportFontMedium, 1, Element.ALIGN_RIGHT, Rectangle.NO_BORDER, false);
                WriteCell(detailsTable, Convert.ToDateTime(statusChange["mdh_time"]).ToShortDateString() + " " + Convert.ToDateTime(statusChange["mdh_time"]).ToShortTimeString(), ReportFontMedium, 1, Element.ALIGN_CENTER, Rectangle.NO_BORDER, false);
                WriteCell(detailsTable, statusChange["new_status"].ToString(), ReportFontMedium, 1, Element.ALIGN_RIGHT, Rectangle.NO_BORDER, false);
                WriteCell(detailsTable, "", ReportFontBold, 3, Element.ALIGN_RIGHT, Rectangle.NO_BORDER);
        ...
        private void ReportColumns(PdfPTable columnsTable)
        {
            ...
            WriteCell(columnsTable, "Status", ReportFontBold, 1, Element.ALIGN_RIGHT, Rectangle.NO_BORDER);
            WriteCell(columnsTable, "", ReportFontBold, 3, Element.ALIGN_RIGHT, Rectangle.NO_BORDER);

            WriteCell(columnsTable, "", ReportFontBold, 6, Element.ALIGN_RIGHT, Rectangle.BOTTOM_BORDER);
        }

        private void ReportHeader(PdfPTable headingtable, Image gif, StatusChangeReport pageEvent, int colspan)
        {
            ...
            WriteCell(headingtable, "Status Change", ReportFontHeading, 6, Element.ALIGN_CENTER, Rectangle.NO_BORDER);
        ...
        public override void OnOpenDocument(PdfWriter writer, iTextSharp.text.Document document)
        {
            try
            {
                ...
                //ReportHeader(headerTbl, logo, (StatusChangeReport)writer.PageEvent);
        ...
        public override void OnStartPage(PdfWriter writer, iTextSharp.text.Document document)
        {
            ...
            try
            {
                ...
                //I used a large version of the logo to maintain the quality when the size was reduced. I guess you could reduce the size manually and use a smaller version, but I used iTextSharp to reduce the scale. As you can see, I reduced it down to 7% of original size.
                logo.ScalePercent(25);
                ReportHeader(headerTbl, logo, (StatusChangeReport)writer.PageEvent, 6);
