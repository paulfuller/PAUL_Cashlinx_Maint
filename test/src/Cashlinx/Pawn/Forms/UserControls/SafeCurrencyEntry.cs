...
* and tube safe denominations for safe
* Sreelatha Rengarajan 9/28/2010 Initial version
*******************************************************************/
...
{
    public partial class SafeCurrencyEntry : UserControl
    {
        ...
        private int tubeDenomCount = 0;

        public SafeCurrencyEntry()
        {
            InitializeComponent();
        }
        ...
        private void customTextBoxDollar2Amt_Leave(object sender, EventArgs e)
        {
            ...
            if (dollar2Amt > 0.0d)
            {
                ...
                if (Math.Round(quotient) != quotient)
                {
                    MessageBox.Show("Amount entered " + dollar2Amt.ToString() + " is not correct for the denomination");
        ...
        public List<string> CurrencyEntryData()
        {
            ...
            foreach (Control c in tableLayoutPanelTubeDenom.Controls)
            {
                const string labelName = "label_amount_";

                if (c.Name.Contains(labelName))
                {
                    if (!string.IsNullOrEmpty(c.Text) && Utilities.GetDecimalValue(c.Text, 0) > 0)
                    {
                        string displayText = c.Name.Substring(13);
                        currencyData.Add(displayText);
                        currencyData.Add(c.Text);
                    }
                }
                else
                    continue;
        ...
        {
            //Remove the $ sign from the data in coin total and dollar total and tube total
            ...
            string tubeData = customTextBoxTubeTotal.Text;
            if (tubeData.StartsWith("$"))
                tubeData = tubeData.Substring(1);
            decimal tubeTotal = Utilities.GetDecimalValue(tubeData, 0);
            ...
            if (coinTotal >= 0 || dollarTotal >= 0 || tubeTotal >= 0 || otherTenderTotal >= 0)
            {
                Calculate(coinTotal + dollarTotal + tubeTotal + otherTenderTotal);
            }
        }
        ...
        void custTextCount_Leave(object sender, EventArgs e)
        {
            string name = ((CustomTextBox)sender).Name;
            string labelName = "label_amount_" + name;
            tableLayoutPanelTubeDenom.Controls[labelName].Text = (Utilities.GetIntegerValue(name, 0) * Utilities.GetIntegerValue(((CustomTextBox)sender).Text, 0)).ToString();
            decimal totTubeAmount = 0.0m;
            foreach (Control ctrl in tableLayoutPanelTubeDenom.Controls)
            {
                if (ctrl.GetType() == typeof(Label) && ctrl.Name.Contains("label_amount_"))
                {
                    string tubeData = ctrl.Text;
                    decimal tubeAmount = Utilities.GetDecimalValue(tubeData, 0);
                    totTubeAmount += tubeAmount;
                }
            }
            customTextBoxTubeTotal.Text = string.Format("{0:C}", totTubeAmount);
        }
        ...
        void otControl_AmtContinue(decimal otherTenderTotal, int qnty, List<decimal> otherTenderDetails)
        {
            customTextBoxOTAmount.Text = string.Format("{0:C}", otherTenderTotal);
            customTextBoxOTCount.Text = qnty.ToString();
            otherTenderData = new List<decimal>();
            otherTenderData = otherTenderDetails;
