...
{
    /// <summary>
    /// Utility methods for reading/writing USB key data and
    /// for detecting if a USB key is available or not
    /// Travis Riffle / Marshall Moorman
    /// </summary>
    public static class USBUtilities
    {
        public class USBDriveStorage
        {
            ...
            public USBDriveStorage()
            {
                Obliterate();
            }

            public void Populate(string data, string serNumber, string dPath)
            {
                Obliterate();
                if (!string.IsNullOrEmpty(serNumber))
                {
                    this.driveSerialNumber = serNumber;
                    this.driveData = null;
                    this.drivePath = null;
                }
                ...
                {
                    this.driveData = data;
                    if (!string.IsNullOrEmpty(this.driveSerialNumber) &&
                        !string.IsNullOrEmpty(this.drivePath))
                    {
                        IsValid = true;
                        return;
                    }
            ...
            public void PopulateNewData(string data)
            {
                if (!IsValid && (string.IsNullOrEmpty(this.driveSerialNumber) || string.IsNullOrEmpty(this.drivePath)))
                    return;

                if (!string.IsNullOrEmpty(data))
                {
                    this.Populate(data, this.driveSerialNumber, this.drivePath);
                }
            }
        ...
        public static string GetVolumeSerial(string strDriveLetter)
        {
            ...
            GetVolumeInformation(strDriveLetter, VolLabel, (UInt32)VolLabel.Capacity, ref serNum, ref maxCompLen, ref VolFlags, FSName, (UInt32)FSName.Capacity);
                    ...
                    switch (m.WParam.ToInt32())
                    {
                        ...
                        case DBT_DEVICEQUERYREMOVE:
                            ...
                            if (devType == DBT_DEVTYP_HANDLE)
                            {
                                ...
                                {
                                    DriveDetectorEventArgs e = new DriveDetectorEventArgs();
                                    e.Drive = HookedDrive;        // drive which is hooked
                                    tempQuery(this, e);
