...
namespace Common.Libraries.Utility.BarcodeGenerator.Symbologies
{
    class EAN8 : BarcodeCommon, IBarcode
    {
        ...
        {
            //check length
            if (Raw_Data.Length != 8 && Raw_Data.Length != 7) throw new System.Exception("EEAN8-1: Invalid data length. (7 or 8 numbers only)");

            //check numeric only
            if (!Barcode.CheckNumericOnly(Raw_Data)) throw new System.Exception("EEAN8-2: Numeric only.");
